...
export function Sidebar({ currentView, onNavigate, onAddRecord, onLogout }: SidebarProps) {
  return (
    <aside className="h-screen w-64 fixed left-0 top-0 z-40 bg-surface-container-low border-r border-outline-variant/20 flex flex-col p-4 gap-2">
      ...
      <button
        type="button"
        onClick={onAddRecord}
        className="mt-4 mb-8 mx-2 py-2.5 bg-gradient-to-r from-primary to-primary-container text-white rounded-md font-medium text-sm shadow-md active:scale-95 transition-all flex items-center justify-center gap-2"
      >
      ...
      <div className="mt-auto border-t border-outline-variant/20 pt-4 flex flex-col gap-1">
        <button
          type="button"
          onClick={onLogout}
          className="flex items-center gap-3 px-4 py-2 text-on-surface-variant hover:text-error hover:bg-surface-container/50 rounded-lg transition-all w-full text-left"
        >
